...
export async function POST(
    ...
    try {
        ...
        if (!workflowsResponse.ok) {
            throw new Error(
                `Failed to fetch workflows: ${workflowsResponse.status} ${workflowsResponse.statusText}`
            );
        }
        ...
        if (workflowsData.data && Array.isArray(workflowsData.data)) {
            targetWorkflow = workflowsData.data.find(
                (item: Record<string, unknown>) => {
                    const workflowContentId = (
                    ...
                    return (
                        (workflowContentId?.id as string) === id ||
                        workflowName
                            ?.replace(/^workflows\//, '')
                            .replace(/\.json$/, '') === id
                    );
        ...
        const workflow = JSON.parse(
            JSON.stringify(targetWorkflow.content.content)
        );

        console.log('✅ Found workflow:', targetWorkflow.content.name);
        console.log(
            '🔍 Original workflow nodes count:',
            workflow.nodes?.length || 0
        );
        console.log(
            '🔍 First node before processing:',
            JSON.stringify(workflow.nodes?.[0], null, 2)
        );
        ...
        console.log(
            '🔍 First node after processing:',
            JSON.stringify(processedWorkflow.nodes?.[0], null, 2)
        );
 ...
 */
function processWorkflowPrompts(
    ...
    if (processedWorkflow.nodes && Array.isArray(processedWorkflow.nodes)) {
        console.log(
            '🔄 Processing',
            processedWorkflow.nodes.length,
            'nodes...'
        );
        ...
        processedWorkflow.nodes.forEach((node: Record<string, unknown>) => {
            ...
            if (node.type && !node.class_type) {
                console.log(
                    `🔧 Converting node ${node.id}: type "${node.type}" -> class_type`
                );
                node.class_type = node.type;
                delete node.type;
            } else if (!node.class_type && !node.type) {
                console.error(
                    `❌ Node ${node.id} missing both type and class_type!`
                );
            }
            ...
            if (
                ...
                (!node.title ||
                    (typeof node.title === 'string' &&
                        (node.title.toLowerCase().includes('positif') ||
                            !node.title.toLowerCase().includes('negatif'))))
            ) {
            ...
            if (
                ...
                console.log(
                    '⚙️ Updating steps in KSampler node',
                    node.id,
                    'from',
            ...
            if (
                node.class_type === 'FluxGuidance' &&
                node.widgets_values &&
                Array.isArray(node.widgets_values) &&
                node.widgets_values.length > 0
            ) {
                console.log(
                    '🎯 Updating guidance in FluxGuidance node',
                    node.id,
                    'from',
                    node.widgets_values[0],
                    'to',
            ...
            if (
                ...
                const currentText =
                    typeof node.widgets_values[0] === 'string'
                        ? node.widgets_values[0].toLowerCase()
                        : '';
                if (
                    currentText.includes('text, watermark') ||
        ...
        const nodesWithoutClassType = processedWorkflow.nodes.filter(
            (node: Record<string, unknown>) => !node.class_type
        );
        const nodesWithInvalidIds = processedWorkflow.nodes.filter(
            (node: Record<string, unknown>) =>
                !node.id ||
                node.id === '#id' ||
                (typeof node.id === 'string' &&
                    (node.id as string).includes('#'))
        );

        if (nodesWithoutClassType.length > 0) {
            console.error(
                '❌ Found nodes without class_type after processing:',
                nodesWithoutClassType.map((n: Record<string, unknown>) => ({
                    id: n.id,
                    type: n.type,
                }))
            );
            throw new Error(
                `${nodesWithoutClassType.length} nodes are missing class_type property`
            );
        }

        if (nodesWithInvalidIds.length > 0) {
            console.error(
                '❌ Found nodes with invalid IDs:',
                nodesWithInvalidIds.map((n: Record<string, unknown>) => ({
                    id: n.id,
                    class_type: n.class_type,
                }))
            );
            throw new Error(
                `${nodesWithInvalidIds.length} nodes have invalid IDs`
            );
        }
        ...
        console.log(
            '✅ All',
            processedWorkflow.nodes.length,
            'nodes have class_type property'
        );
        ...
        processedWorkflow.nodes.forEach((node: Record<string, unknown>) => {
            ...
            if (inputs && Array.isArray(inputs)) {
                (inputs as Record<string, unknown>[]).forEach(
                    (input: Record<string, unknown>) => {
                        if (input.link !== null && input.link !== undefined) {
                            // Find the source node for this connection
                            const sourceLink = (
                                processedWorkflow.links as unknown[]
                            )?.find(
                                (link: unknown) =>
                                    Array.isArray(link) &&
                                    (link as unknown[])[0] === input.link
                            );
                            if (sourceLink && Array.isArray(sourceLink)) {
                                ...
                                const sourceNode = processedWorkflow.nodes.find(
                                ...
                                if (
                                    ...
                                    (sourceNode.outputs as unknown[])[
                                        sourceSlot as number
                                    ]
                                ) {
                                    (
                                        apiWorkflow[id as string]
            ...
            if (widgets_values && Array.isArray(widgets_values)) {
                ...
                const nodeInputs = apiWorkflow[id as string].inputs as Record<
                    string,
                    unknown
                >;
        ...
        console.log(
            '🔄 Converted to ComfyUI API format with',
            Object.keys(apiWorkflow).length,
            'nodes'
        );
        console.log(
            '🔍 Sample API node:',
            JSON.stringify(Object.entries(apiWorkflow)[0], null, 2)
        );
