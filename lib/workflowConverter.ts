...
export function convertWorkflowToApi(
    ...
    const linkMap = new Map<
        number,
        { sourceNodeId: number; sourceOutputIndex: number; targetType: string }
    >();
    ...
    for (const node of normalWorkflow.nodes) {
        ...
        const nodeType =
            node.type ||
            (node as unknown as Record<string, unknown>).class_type ||
            (node.properties && node.properties['Node name for S&R']);

        if (!nodeType || typeof nodeType !== 'string') {
            console.log(
                `⚠️  Skipping node ${node.id} - no type information found`
            );
            continue;
        }
        ...
        if (node.mode === 4 || isUIOnlyNode(nodeType)) {
            console.log(
                `⏭️  Skipping node ${node.id} (${nodeType}) - mode: ${node.mode}`
            );
            continue;
        }
        ...
        if (node.mode === 2) {
            console.log(
                `⚠️  BYPASSED NODE ${node.id} (${nodeType}) - keeping as potential dependency`
            );
        }

        console.log(
            `✅ Processing node ${node.id} (${nodeType}) - mode: ${node.mode || 0}`
        );
        ...
        if (
            ...
            console.log(
                `🎯 CLIPTextEncode node ${node.id} prompt text: "${promptText.slice(0, 50)}..."`
            );
        }
        ...
        const missingParams = requiredParams.filter(
            param =>
                apiNode.inputs[param] === undefined ||
                apiNode.inputs[param] === null
        );

        if (missingParams.length > 0) {
            console.warn(
                `⚠️  Node ${node.id} (${node.type}) missing required parameters:`,
                missingParams
            );
            console.warn(
                `   Available parameters:`,
                Object.keys(apiNode.inputs)
            );
            ...
            if (node.type === 'FluxResolutionNode') {
                ...
                if (!apiNode.inputs.aspect_ratio)
                    apiNode.inputs.aspect_ratio = '1:1 (Square)';
                if (!apiNode.inputs.divisible_by)
                    apiNode.inputs.divisible_by = '64';
                if (apiNode.inputs.custom_ratio === undefined)
                    apiNode.inputs.custom_ratio = false;
                if (!apiNode.inputs.custom_aspect_ratio)
                    apiNode.inputs.custom_aspect_ratio = '1:1';

                console.log(
                    `🔧 Applied FluxResolutionNode defaults to node ${node.id}`
                );
            }
        } else {
            console.log(
                `✅ Node ${node.id} (${node.type}) has all required parameters`
            );
        }
...
function inferWidgetMappingFromNode(nodeType: string): string[] {
    ...
    if (!nodeType || typeof nodeType !== 'string') {
        console.log(
            `⚠️  Node type is undefined/invalid: ${nodeType}, returning empty mapping`
        );
        return [];
    }
 ...
 */
function handleSpecialNodeCases(
    node: NormalWorkflowNode,
    ...
    switch (node.type) {
        ...
        case 'FluxResolutionNode':
            ...
            console.log(
                `🔧 Processing FluxResolutionNode ${node.id} with ${widgetValues.length} widgets:`,
                widgetValues
            );
            ...
            if (widgetValues.length >= 5) {
                ...
                console.log(
                    `✅ FluxResolutionNode ${node.id} parameters set:`,
                    {
                        megapixel,
                        aspectRatio,
                        divisibleBy,
                ...
                console.error(
                    `❌ FluxResolutionNode ${node.id} missing widgets! Expected 5, got ${widgetValues.length}`
                );
                ...
                console.log(
                    `🔧 Using default FluxResolutionNode parameters for node ${node.id}`
                );
            }
            break;
        ...
        case 'UltimateSDUpscale':
            ...
            if (widgetValues.length >= 21) {
                ...
                if (
                    widgetValues[2] === 'randomize' ||
                    widgetValues[2] === 'fixed'
                ) {
                ...
                for (
                    let i = startIndex;
                    i < Math.min(widgetValues.length, mappingOrder.length);
                    i++
                ) {
...
export function validateApiWorkflow(workflow: ApiWorkflow): boolean {
    ...
    for (const [nodeId, node] of Object.entries(workflow)) {
        if (!node.class_type || !node.inputs) {
            console.error(
                `❌ Invalid node ${nodeId}: missing class_type or inputs`
            );
            return false;
        }
        ...
        if (missingParams.length > 0) {
            console.error(
                `❌ Node ${nodeId} (${node.class_type}) missing required parameters: ${missingParams.join(', ')}`
            );
            console.error(
                `   Available parameters: ${Object.keys(node.inputs).join(', ')}`
            );
            // Don't fail validation, just warn - some parameters might be optional
            console.warn(
                `⚠️  This might cause API errors - check if node should be bypassed/muted`
            );
        }
    }
...
function processNodeInputs(
    ...
    const widgetValues = node.widgets_values || [];
    const extractedNodeType = String(
        nodeType ||
            node.type ||
            (node as unknown as Record<string, unknown>).class_type ||
            (node.properties && node.properties['Node name for S&R']) ||
    ...
    if (!hasSpecialHandling) {
        ...
        if (widgetMapping.length > 0) {
            ...
            for (
                let i = 0;
                i < Math.min(widgetValues.length, widgetMapping.length);
                i++
            ) {
                const paramName = widgetMapping[i];
                if (
                    paramName &&
                    !apiNode.inputs[paramName] &&
                    widgetValues[i] !== undefined
                ) {
                    // Skip control values like "randomize", "fixed"
                    if (
                        typeof widgetValues[i] === 'string' &&
                        [
                            'randomize',
                            'fixed',
                            'increment',
                            'decrement',
                        ].includes(String(widgetValues[i]))
                    ) {
                        continue;
                    }
                    apiNode.inputs[paramName] = widgetValues[i];
            ...
            for (
                let i = 0;
                i < Math.min(widgetValues.length, inputNames.length);
                i++
            ) {
            ...
            for (let i = 0; i < widgetValues.length; i++) {
                if (
                    widgetValues[i] !== undefined &&
                    !['randomize', 'fixed', 'increment', 'decrement'].includes(
                        String(widgetValues[i])
                    )
                ) {
...
function normalizeWorkflowInput(workflow: unknown): NormalWorkflow | null {
    ...
    if (
        ...
        if (keys.length > 0) {
            ...
            const firstNode = (workflow as Record<string, unknown>)[
                firstKey
            ] as Record<string, unknown>;
            if (
                firstNode &&
                typeof firstNode === 'object' &&
                'class_type' in firstNode
            ) {
...
export function processWorkflow(input: unknown): ApiWorkflow | null {
    try {
        ...
        console.log('📋 Detailed input analysis:', {
            ...
            nodeDetails:
                normalWorkflow.nodes?.map(n => ({
                    id: n.id,
                    type: n.type,
                    mode: n.mode || 0,
                    hasWidgets: !!(
                        n.widgets_values && n.widgets_values.length > 0
                    ),
        ...
        const activeModes = normalWorkflow.nodes.reduce(
            (acc, node) => {
        ...
        console.log('📊 Input workflow analysis:', {
            ...
            nodeTypes: [
                ...new Set(normalWorkflow.nodes.map(n => n.type)),
            ].slice(0, 10),
        });
            ...
            for (const [inputName, inputValue] of Object.entries(
                ...
                if (Array.isArray(inputValue) && inputValue.length >= 2) {
                    ...
                    if (filteredNodeIds.includes(sourceNodeId)) {
                        console.log(
                            `🔧 Fixing broken connection in node ${nodeId}.${inputName} -> was pointing to filtered node ${sourceNodeId}`
                        );
                        ...
                        if (inputName === 'model' && sourceNodeId === '56') {
                            if (apiWorkflow['38']) {
                                // UNETLoader
                                console.log(
                                    `  ✅ Redirecting model connection from LoRA (${sourceNodeId}) to UNETLoader (38)`
                                );
                        ...
                        if (!fixed) {
                            console.log(
                                `  ❌ No alternative found, removing connection`
                            );
