...
async function testConverter() {
    try {
        ...
        const normalWorkflowPath = path.join(
            process.cwd(),
            'app/api/proxy/example-normal.json'
        );
        const apiWorkflowPath = path.join(
            process.cwd(),
            'app/api/proxy/example-api.json'
        );

        console.log('📂 Loading example files...');
        const normalWorkflow = JSON.parse(
            fs.readFileSync(normalWorkflowPath, 'utf-8')
        );
        const expectedApiWorkflow = JSON.parse(
            fs.readFileSync(apiWorkflowPath, 'utf-8')
        );
        ...
        console.log(
            `Node count match: ${expectedNodes === convertedNodes ? '✅' : '❌'}`
        );
        ...
        for (const nodeId of keyNodes) {
            ...
            console.log(
                `Node ${nodeId}: Expected: ${expectedExists ? '✅' : '❌'}, Converted: ${convertedExists ? '✅' : '❌'}`
            );
            ...
            if (expectedExists && convertedExists) {
                ...
                console.log(
                    `  - Type match: ${expectedType === convertedType ? '✅' : '❌'} (${convertedType})`
                );
            }
        }
        ...
        if (convertedWorkflow['31']) {
            ...
            console.log(
                `  - Has seed: ${ksampler.inputs.seed !== undefined ? '✅' : '❌'}`
            );
            console.log(
                `  - Has steps: ${ksampler.inputs.steps !== undefined ? '✅' : '❌'}`
            );
            console.log(
                `  - Has model connection: ${Array.isArray(ksampler.inputs.model) ? '✅' : '❌'}`
            );
            console.log(
                `  - Has positive conditioning: ${Array.isArray(ksampler.inputs.positive) ? '✅' : '❌'}`
            );
        }
        ...
        if (convertedWorkflow['45']) {
            ...
            console.log(
                `  - Has text input: ${clipEncode.inputs.text !== undefined ? '✅' : '❌'}`
            );
            console.log(
                `  - Has clip connection: ${Array.isArray(clipEncode.inputs.clip) ? '✅' : '❌'}`
            );
        }
        ...
        const outputPath = path.join(
            process.cwd(),
            'converted-workflow-test.json'
        );
        fs.writeFileSync(
            outputPath,
            JSON.stringify(convertedWorkflow, null, 2)
        );
