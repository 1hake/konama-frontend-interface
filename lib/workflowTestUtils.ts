...
export function testWorkflowConversion() {
    try {
        ...
        const expectedApiWorkflowPath = path.join(
            process.cwd(),
            'app/api/proxy/example-api.json'
        );

        const normalWorkflow = JSON.parse(
            fs.readFileSync(normalWorkflowPath, 'utf8')
        );
        const expectedApiWorkflow = JSON.parse(
            fs.readFileSync(expectedApiWorkflowPath, 'utf8')
        );
        ...
        console.log(
            '📊 Expected nodes:',
            Object.keys(expectedApiWorkflow).length
        );
        console.log(
            '📊 Converted nodes:',
            Object.keys(convertedWorkflow).length
        );
        ...
        for (const nodeId of keyNodes) {
            if (expectedApiWorkflow[nodeId] && convertedWorkflow[nodeId]) {
                console.log(
                    `✅ Node ${nodeId} (${expectedApiWorkflow[nodeId].class_type}) present in both`
                );
            } else if (expectedApiWorkflow[nodeId]) {
                console.log(
                    `⚠️  Node ${nodeId} (${expectedApiWorkflow[nodeId].class_type}) missing from conversion`
                );
            }
        }
        ...
        const outputPath = path.join(
            process.cwd(),
            'app/api/proxy/converted-test.json'
        );
        fs.writeFileSync(
            outputPath,
...
export function validateWorkflowStructure(workflow: unknown): {
    ...
    if (
        ...
        for (let i = 0; i < workflowObj.nodes.length; i++) {
            ...
            if (node.widgets_values && !Array.isArray(node.widgets_values)) {
                errors.push(
                    `Node ${i} (${nodeType}) widgets_values must be an array`
                );
            }
        }
    ...
    errors.push(
        'Workflow format not recognized (neither normal nor API format)'
    );
    return { isValid: false, errors };
}
